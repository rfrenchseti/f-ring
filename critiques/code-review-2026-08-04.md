...
### 1.1 `fit_quadratic` evaluates the model with constant and quadratic coefficients swapped
...
### 1.2 The user guide's core-radius equation is missing the longitude term
...
### 2.1 `et_to_tour` compares YMD strings against DOY cutoffs → wrong `mission_phase_name`
...
### 2.2 Mosaic per-column ET is float32 → archived times and inertial longitudes quantized
...
### 2.3 Labels declare `vertical_display_direction` "Top to Bottom" but row 0 = inner radius
...
### 2.4 `document_standard_id` "Python" is not a valid PDS4 enumeration
...
### 2.5 `collection_document.lblx` hardcodes `Inventory` `<records>5</records>` — CSV has 10 rows
...
### 2.6 Collection inventories can disagree with the products actually generated
...
### 2.7 Index-only runs silently produce empty global indexes
...
### 2.8 Reproj labels reference `SUPPL_PATH` that is only set when suppl files are generated
...
### 3.1 Viewer filter computes moon separation from the wrong orbital extreme
...
### 3.2 Background staleness check ignores the products downstream actually consumes
...
### 3.3 "Display Background" crashes for any obsid without a computed background
...
### 3.4 Missing offset file → image silently reprojected at zero offset
...
### 3.5 `f_ring_util` API breaks against its own callers (3 crashes)
...
### 3.6 `ring_plot_offsets.py` crashes on obsids lacking a navigation type
...
### 3.7 `FILELIST_BRING_MOUNTAINS/defaults.txt` is short two lines → startup crash for that type
...
### 4.2 Product counts are stale after the joint-observation split
...
### 4.3 Both quick-start commands are broken as printed
...
### 4.4 "Each line refers, in sequence, to an X index in the image array" is false in two cases
...
### 4.5 `core_radius` is described as "(constant for one image)"
...
### 4.6 Broken hyperlink target left over from the docx conversion
...
- **`INST_CMPRS_PARAM` is prepared (with a deliberate `'N/A'→'999'` transform) but no template
  ...
  drift; author lists are hardcoded per-template in ~8 places.
...
- `collection_document.csv:3-10` / `collection_miscellaneous.csv:4-12`: context products (and one
  ...
  inconsistent with `collection_context.csv`.
...
- `nav/ring_mosaic.py:1143` (upstream): `mean_incidence` is overwritten by *each added image*, so
...
- `ring_make_mosaic_list.py:25-29`: re-`ring_init` after the first ring type is a no-op for
  ...
  `mosaics/obs_list.csv` is a stale snapshot (302 obsids; missing the three split additions).
- All F-ring `defaults.txt` files carry an unread 9th line (`1`) — `ring_init` stops at 8;
  visually matches the hardcoded `_1` filename suffix but is not parsed (trap).
...
4. **Guide edits for the next PDF build:** §1.2 equation (most important — published math),
   §4.2–§4.6, plus the §5 guide minors.
