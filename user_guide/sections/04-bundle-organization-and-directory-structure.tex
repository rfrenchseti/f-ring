...
The image file is a pure binary file with no headers or record separators. Data is stored in LSB IEEE754 single-precision (32-bit) floating point format. The Y axis (Δ\textit{r}) always has size 401 (which covers the radial distance from the core of −1000 km to +1000 km in steps of 5 km). The X axis (corotating longitude) has a variable size depending on the number of longitudes that contain valid data (see Section \ref{sec:reprojection}). Any array elements that do not contain valid data will be marked with the value −999. See Section \ref{sec:reading-labels-and-data-product-files} for information on how to read these files with software.
