...
class ImageDisp(tk.Frame):
    ...
    def __init__(self,
                 ...
                 enlarge_limit=None, one_zoom=True,
                 auto_update=True,
        ...
        enlarge_limit  The maximum magnification permitted beyond unit zoom.
                       This can be a tuple (max_x, max_y).
...
#                 return False
    ...
    def _mousemove_callback_handler(self, event, img_num, callback_func):
        ...
        if (x < 0 or y < 0 or
            ...
            y >= self.imgdata_list[img_num].shape[0]):
            return
        ndigits_x = len(str(self.imgdata_list[img_num].shape[1]-1))
        ndigits_y = len(str(self.imgdata_list[img_num].shape[0]-1))
        self.label_xy.config(text=('Mouse coord: %'+str(ndigits_x+3)+
