...
#Define variables
region = 'iter_10' #currently just using the 10km slices
...
rins = np.arange(139220, 141220, 10)
routs = rins + 9 #outer radius should be inner + 10km
...
for i in range(len(rins)):
    ...
    if rin >= rout:
        ...
        #calculate mu, mu0
        orig_obsdata['Mu'] = np.abs(np.cos(np.radians(orig_obsdata['Mean Emission'])))
        orig_obsdata['Mu0'] = np.abs(np.cos(np.radians(orig_obsdata['Incidence'])))
