...
paper_root = os.path.join(ringutil.PAPER_ROOT, 'Talk-RWG2014')
root_clump_db = {}
...
def plot_long_coverage_over_time():
    ...
    for obsid in sorted_ids:
        ...
        plt.plot(time, longitudes, color = color_cassini, lw=2)
...
#     ax.set_xlabel('Observation Date')
    ax.set_ylabel('Longitude ( $\mathbf{^o}$)')
#     fig.tight_layout()
    fig.autofmt_xdate()
...
#        ax.xaxis.set_major_formatter(xFormatter)
        ...
        if left_idx_max < len(ew_data):
            ax.plot(tri_longs[left_idx_max:len(ew_data)]-360., tri_ews[left_idx_max:len(ew_data)], color = 'white', lw = 2.0)
            ax.plot(tri_longs[len(ew_data):right_idx_max+1], tri_ews[len(ew_data):right_idx_max+1], color = 'white', lw = 2.0)
        else:
            ax.plot(tri_longs[left_idx_max:right_idx_max + 1],tri_ews[left_idx_max:right_idx_max + 1], color = 'white', lw = 2.0)
            
        if left_idx < len(ew_data):
            ax.plot(tri_longs[left_idx:len(ew_data)]-360., tri_ews[left_idx:len(ew_data)], color = '#F74F4F', lw = 3)
            ax.plot(tri_longs[len(ew_data):right_idx+1], tri_ews[len(ew_data):right_idx+1], color = '#F74F4F', lw = 3)
        else:
            ax.plot(tri_longs[left_idx:right_idx + 1],tri_ews[left_idx:right_idx +1], color = '#F74F4F', lw = 3)
            l, = ax.plot(g_long_range, gauss, color = 'yellow', ls = ':', lw = 1.5)
            l.set_dashes([3,2])
...
def plot_voyager_cassini_comparison_profiles(adjust):
...
#                  'ISS_055RI_LPMRDFMOV001_PRIME', 'ISS_00ARI_SPKMOVPER001_PRIME']
    ...
    for use_baseline in [False, True]:
        for max_plot_phase in [0,1,2]:
            ...
            if use_baseline:
                ax.set_ylabel('Baseline-Normalized Equiv Width')
                ax.set_ylim(0,9)
                ...
                ax.set_ylabel(r'Phase-Normalized Equiv Width (km)')
#                ax.set_ylim(0,3.5)
#                ax.get_yaxis().set_ticks([0,3.5])
#                ax.get_yaxis().set_ticks([0.5,1,1.5,2,2.5,3], minor=True)
                ax.set_ylim(0,1.5)
                ax.get_yaxis().set_ticks([0,1.5])
                ax.get_yaxis().set_ticks([0.5,1,1.5], minor=True)
            ax.set_xlabel(r'Co-Rotating Longitude ( $\mathbf{^o}$)')
            ax.set_xlim(0,360.)
            ax.get_xaxis().set_ticks([0,360])
            ...
            cassini_list = []
            for plot_phase in range(max_plot_phase+1):
                for obs_id in obsid_list:
                    ...
                    if (obs_id == 'ISS_036RF_FMOVIE001_VIMS' or
                        ...
                        continue
                    if adjust:
                        c_ew_data -= baseline_value(c_ew_data)
            ...
            for v_obs in v_all_clump_db.keys():
                ...
                if v_obs[:2] == 'V1':
                    ...
                    alpha = 0.9
                if adjust:
                    v_ew_data -= baseline_value(v_ew_data)
            ...
            leg = ax.legend([voyager1[0], cassini_list[0]],
                            ['Voyager', 'Cassini'], loc = 1,
                            handlelength=2)
            leg.get_frame().set_alpha(0.0)
            leg.get_frame().set_visible(False)
            ...
            if use_baseline:
                save_fig(fig, ax, 'voyager_cassini_profile_comparison_'+str(max_plot_phase)+'_'+str(adjust)+'_baseline.png', leg)
            else:
                save_fig(fig, ax, 'voyager_cassini_profile_comparison_'+str(max_plot_phase)+'_'+str(adjust)+'.png', leg)
...
def plot_voyager_cassini_comparison_profiles2(adjust):
    
#    obsid_list = ['ISS_000RI_SATSRCHAP001_PRIME','ISS_059RF_FMOVIE002_VIMS','ISS_075RF_FMOVIE002_VIMS', 
#                  'ISS_055RI_LPMRDFMOV001_PRIME', 'ISS_00ARI_SPKMOVPER001_PRIME']

    pcolors = ['#ff0000', '#ff2000', '#ff4000', '#ff6000', '#ff8000', '#ffa000', '#ffc000', '#ffff00']

    first_pass = True
                
    for use_baseline in [False, True]:
        for max_plot_phase in [0,1,2,3]:
            obsid_list = c_all_clump_db.keys()
...
#            ax.set_xlabel(r'Co-Rotating Longitude ( $\mathbf{^o}$)')
            ...
            cassini_list = []
            cassini_label = []
            for plot_phase in [max_plot_phase]:
                for obs_id in sorted(obsid_list):
                    if not use_baseline and c_all_clump_db[obs_id].incidence_angle > 87:
                        print 'V/C COMPARISON PROFILES SKIPPING', obs_id
                        continue
                    c_ew_data = c_all_clump_db[obs_id].ew_data
                    ...
                    if (obs_id == 'ISS_036RF_FMOVIE001_VIMS' or
                        ...
                        if plot_phase != 1:
                            continue
                        marker = '-'
                        color = pcolors[cnum] # Monster
                        cnum += 1
                        lw = lw_profile
                        alpha = 0.8
                    elif (obs_id == 'ISS_105RI_TDIFS20HP001_CIRS' or
                          obs_id == 'ISS_105RF_FMOVIE002_PRIME' or
                          obs_id == 'ISS_106RF_FMOVIE002_PRIME' or
                          obs_id == 'ISS_107RF_FMOVIE002_PRIME' or
                          obs_id == 'ISS_108RI_SPKMVLFLP001_PRIME'):
#                          obs_id == 'ISS_108RF_FMOVIE001_PRIME'):
                        if plot_phase != 2:
                            continue
                        marker = '-'
                        color = pcolors[int(cnum)] # 2009
                        cnum += 1.5
                        if cnum > 7: cnum=7
                        lw = lw_profile
                        alpha = 0.8
...
#                    if plot_phase == 0:
            ...
            leg = None
#            if plot_phase == 0 or plot_phase == 1 or plot_phase == 2:
#                if len(cassini_list) > 0:
#                    leg = ax.legend([cassini_list[0], cassini_list[-1]],
#                                    cassini_label[0], cassini_label[-1])
#            elif plot_phase == 3:               
#                leg = ax.legend([voyager1[0], voyager2[0]],
#                                ['Voyager 1', 'Voyager 2'], loc = 1)
#            if leg is not None:
#                leg.get_frame().set_alpha(0.0)
#                leg.get_frame().set_visible(False)
            ...
            if use_baseline:
                save_fig(fig, ax, 'voyager_cassini_profile_comparison2_'+str(max_plot_phase)+'_'+str(adjust)+'_baseline.png', leg)
            else:
                save_fig(fig, ax, 'voyager_cassini_profile_comparison2_'+str(max_plot_phase)+'_'+str(adjust)+'.png', leg)
...
def plot_combined_vel_hist():
    ...
    plt.subplots_adjust(top = .97, bottom = 0.15, left = 0.08, right = 0.98)
...
#         plt.errorbar(phase_list, ew_list, yerr=std_list, ecolor=mec, fmt=fmt, ms=ms, mec=mec, mfc=mfc, mew=0.5, label=label)
        plt.plot(np.arange(0,180), 10**scale*10**np.polyval(coeffs, np.arange(0,180)), ls, lw=3, color=mec, label=label)
    ...
    c_base_ew_list = ringutil.compute_corrected_ew([x.baseline_ew for x in obsdata_db['C']],
                                          ...
                                          c_tau_base)
    c_peak_ew_list = ringutil.compute_corrected_ew([x.peak_ew for x in obsdata_db['C']],
                                          [x.mean_emission_angle for x in obsdata_db['C']],
                                          [x.mean_incidence_angle for x in obsdata_db['C']],
                                          c_tau_base)
    ...
    for v_prof_key in ['V1I', 'V1O', 'V2I', 'V2O']:
        ...
        base_ew_list = ringutil.compute_corrected_ew([x.baseline_ew for x in obsdata_db[v_prof_key]],
                                            ...
                                            v_tau_base)
        peak_ew_list = ringutil.compute_corrected_ew([x.peak_ew for x in obsdata_db[v_prof_key]],
                                            [x.mean_emission_angle for x in obsdata_db[v_prof_key]],
                                            [x.mean_incidence_angle for x in obsdata_db[v_prof_key]],
                                            v_tau_base)
        if v_prof_key[:2] == 'V1':
            ...
            v1io_base_ews.append(base_ew_list[0]*1.34)
            v1io_peak_ews.append(peak_ew_list[0]*1.34)
        else:
            color = color_voyager
            v2io_phase.append(phase_angle[0])
            v2io_base_ews.append(base_ew_list[0]*.97)
            v2io_peak_ews.append(peak_ew_list[0]*.97)

    coeffs = default_coeffs
    print coeffs
    _, _ = plot_phase_curve(c_mean_phase_list, c_base_ew_list, None, None, 'o', ms_phase, '-', 'black', 'black', ' ')
    _, _ = plot_phase_curve(c_mean_phase_list, c_base_ew_list, None, None, 'o', ms_phase, '-', color_cassini, color_cassini, 'Cassini')
    _, v1iob_scale = plot_phase_curve(v1io_phase, v1io_base_ews, None, coeffs, '^', ms_phase, '-', color_voyager1, color_voyager1, 'Voyager 1')
    _, v2iob_scale = plot_phase_curve(v2io_phase, v2io_base_ews, None, coeffs, '^', ms_phase, '-', color_voyager2, color_voyager2, 'Voyager 2')
    
    ax.set_yscale('log')
    plt.xlabel(r'Phase angle ($^\circ$)')
    plt.ylabel(r'$\tau$-Adjusted E.W. (km)')
    plt.title('')
    ax.set_xlim(0,180)
    ax.set_ylim(.08, 50)
...
#     ax.get_yaxis().set_major_formatter(FixedFormatter(['0.1', '1', '10']))    
    ...
    leg = plt.legend(loc='upper left', labelspacing=0.2, fontsize=18)
    ...
    _, v1iob_scale = plot_phase_curve(v1io_phase, v1io_base_ews, None, coeffs, '^', ms_phase, '-', color_voyager1, color_voyager1, 'Voyager 1')
    _, v2iob_scale = plot_phase_curve(v2io_phase, v2io_base_ews, None, coeffs, '^', ms_phase, '-', color_voyager2, color_voyager2, 'Voyager 2')
    _, _ = plot_phase_curve(c_mean_phase_list, c_peak_ew_list, None, coeffs, '^', ms_phase, '--', 'black', 'black', ' ')
    _, c_peak_scale = plot_phase_curve(c_mean_phase_list, c_peak_ew_list, None, coeffs, '^', ms_phase, '--', color_cassini, color_cassini, 'Cassini')
    _, v1iop_scale = plot_phase_curve(v1io_phase, v1io_peak_ews, None, coeffs, '^', ms_phase, '--', color_voyager1, color_voyager1, 'Voyager 1')
    _, v2iop_scale = plot_phase_curve(v2io_phase, v2io_peak_ews, None, coeffs, '^', ms_phase, '--', color_voyager2, color_voyager2, 'Voyager 2')
...
#     ax.get_yaxis().set_major_formatter(FixedFormatter(['0.1', '1', '10']))    
    ...
    leg = plt.legend(loc='upper left', ncol=2, labelspacing=0.2, fontsize=18)

    save_fig(fig, ax, 'phase_curves_peak.png', legend=leg)
...
#        print obsid, i +1
    ...
    plt.hist(dw_dt_list, bins, weights = dw_dt_weights, color=color_cassini)
...
#     ax.get_yaxis().set_ticks([], minor=True)
    ...
    plt.plot(longitudes, ew_data, lw = lw_profile, color = 'white')
    fig.tight_layout()
...
def plot_poisson():    
    ...
    pcolors = ['#ff0000', '#ff3000', '#ff7000', '#ffa000', '#ffc000', '#ffff00']
...
#         if n == 5:
        plt.plot(mean, prob_vgr * prob_cas, '-', lw=3, label='$N='+str(n)+'$', color=pcolors[n-5])
#         else:
#             plt.plot(mean, prob_vgr * prob_cas, ':', label='$N='+str(n)+'$', color='white',
#                      dashes=((n-4),(n-4)))
            
    
    plt.xlabel('Expected Number of Simultaneous Bright Clumps ($\lambda$)')
...
# plot_sample_profile() # DONE
# plot_clump_matrix(['C2', 'C6', 'C9', 'C13', 'C15', 'C30', 'C31', 'C35', 'C39']) # Done
# plot_long_coverage_over_time() # DONE
# plot_combined_vel_hist() # DONE
plot_voyager_cassini_comparison_profiles2(False) # DONE
#plot_voyager_cassini_comparison_profiles2(True) # DONE
# plot_phase_curves() # DONE
#plot_poisson() # DONE
