...
ROOT = '/home/shannon/Documents/Poster_Pics/'
...
def plot_single_clump(ax, ew_data, clump, long_min, long_max, label=False, color=clump_color):
    ...
    ax.plot(idx_range, mexhat, '-', color=color, lw=4, alpha=0.9, label=legend)
    if longitude_idx-clump.scale_idx/2 < 0: # Runs off left side - plot it twice
        ax.plot(idx_range+360, mexhat, '-', color=color, lw=4, alpha=0.9)
...
def plot_single_ew_profile(ax, ew_data, clump_db_entry, long_min, long_max, label=False, color= profile_color):
    ...
    ax.plot(long_range, ew_range, '-', label=legend, color= profile_color, lw = 2)
...
def find_clumps(options, ew_data, ew_mask, longitudes, obs_id, mosaic_data, root):
...
#    print long_res
    ...
    for maximum_scale_idx, maximum_long_idx in zip(xform_maxima_scale_idx, xform_maxima_long_idx):
        ...
        if options.clump_size_max >= scale >= options.clump_size_min:
            ...
            mexhat_base, mexhat_height = sciopt.fmin_powell(fit_wavelet_func,
                                                            (1., 1.),
                                                            args=(restr_mexhat, extracted_data),
...
#    clump_db_entry.smoothed_ew = smoothed_ew
    ...
    if options.plot_scalogram:
        ...
        plt.savefig(os.path.join(ROOT, OBSID + '_scalogram_contours.png'), dpi = 600, transparent = True)
        ...
        ax2.plot(longitudes, ew_data, color = 'black', lw = 2)
        ...
        for clump_data in clump_list:
            if (clump.fit_sigma >= 1.0) and ( 10.0 < clump.scale < 40.0):
                plot_single_clump(ax2, ew_data, clump_data, 0., 360.)
...
def find_clump_chain(options, clump_db):
    ...
    clump_longs = ['224.64', '226.4', '227.12', '227.36']
...
#    plt.show()
    plt.savefig(os.path.join(ROOT, OBSID + '_wavelet.png'), dpi = 300, transparent = True)
...
def compare_clumps_per_obsid(v_clump_db, clump_db):
...
def compare_profiles(v_clump_db, c_clump_db):
    ...
    for v_obs in v_clump_db.keys():
        ...
        c_ew_data = c_clump_db[OBSID].ew_data
        c_ew_data = downsample_ew(c_ew_data)
        ...
        plt.savefig(os.path.join(ROOT, v_obs + 'comparison_profile.png'), dpi = 600, bbox_inches = 'tight', transparent = True)
#        plt.show()
...
#      
    ...
    step = 0.3
    bins = np.arange(0.0,graph_max,step)
    ax.set_xlim(1.0, 6.0)
...
#edit_mosaic(mosaic)
find_clumps(options, smoothed_ew, ew_mask, longitudes, OBSID, mosaic_data, ROOT)
plot_only_profile(longitudes, smoothed_ew)
#plot_clumps(ew_data)
#plot_mexhat(options)
...
#VOYAGER PLOTS
#v_clump_scales, v_clump_heights, c_clump_scales, c_clump_heights = compare_clumps_per_obsid(v_clump_db, ds_clump_db)
