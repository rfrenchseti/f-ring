...
#    plt.show()
    ...
    for func_name in ['Gaussian', 'SDG', 'FDG']:
        if func_name == 'Gaussian':
            sdg_scale = np.std(clump_avg_arr)              #sigma
    ...
    #                                                                               (np.pi/64., np.pi/64., np.pi/64., np.pi/64.),
        ...
        print np.std(mw)
        x = np.arange(-1.,1,.002)
        plt.plot(xi, mw, label=func_name)
        
        y = np.arange(0, 1, .01)
        x_sig = np.zeros(100) + np.std(mw)
        x_2sig = np.zeros(100) + (2*np.std(mw))
        x_3sig = np.zeros(100) + (3*np.std(mw))
        
        plt.plot(x_sig, y, color = 'blue')
        plt.plot(x_2sig, y, color = 'blue')
        plt.plot(x_3sig, y, color = 'blue')
        
        plt.plot(-1*x_sig, y, color = 'blue')
        plt.plot(-1*x_2sig, y, color = 'blue')
        plt.plot(-1*x_3sig, y, color = 'blue')
