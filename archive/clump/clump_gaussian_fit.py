...
def fit_gaussian(tri_ews, clump_left_idx, clump_right_idx, clump_longitude_idx, plot = False):
    """Fit a Gaussian to a clump."""
    # All indexes are assumed to be in accordance to a tripled EW profile to
    # compensate for overlapping
    def fitting_func(params, xi, ew_range, clump_half_width, clump_center,
                     min_ew, max_ew):
        m_angle, s_angle, h_angle, b_angle = params
        base, sigma, height, center_offset = convert_angle(
            b_angle, s_angle, h_angle,m_angle, clump_half_width, min_ew, max_ew)
    ...
    len_ew_data = tri_ews.size//3
    long_res = 360./len_ew_data

    clump_longitude = (clump_longitude_idx - len_ew_data)*long_res
    clump_center_idx = int(np.round((clump_right_idx+clump_left_idx)/2))
...
#    print clump_left_idx, clump_right_idx
    ...
    if old_clump_ews.size < 3:
        # Sometimes the downsampled versions don't end up with enough data to fit
        # a Gaussian to the arrays
        return(0)
...
#    clump_std = ma.std(old_clump_ews)
    ...
    clump_data, residual, array, *_ = sciopt.fmin_powell(
        fitting_func, (0., 0., 0., 0.),
        args=(x, old_clump_ews, clump_half_width_idx*long_res,
              clump_longitude, min_ew, max_ew),
        ftol=1e-8, xtol=1e-8, disp=False, full_output=True)

    m_angle, s_angle, h_angle, b_angle = clump_data
    base, sigma, height, center_offset = convert_angle(
        b_angle, s_angle, h_angle, m_angle, clump_half_width_idx*long_res,
        min_ew, max_ew)
...
#    print left_sigma, right_sigma
    left_sigma_idx = int(np.floor(left_sigma/long_res)) + len_ew_data
    right_sigma_idx = int(np.ceil(right_sigma/long_res)) + len_ew_data
...
#    print len(ew_range), len(ew_longs)
    if len(ew_range) < 3:
        return (0)
    left_long, right_long, clump_int_height, clump_fit_height = find_edges(
        ew_range, gauss, ew_longs, center, sigma, long_res)
...
def refine_fit(clump, ew_data, voyager, downsample):
    long_res = 360./ew_data.size
    longitudes = np.arange(0, 360., long_res)
    tri_long = np.tile(longitudes, 3)
    tri_ew = np.tile(ew_data, 3)
    wav_center_idx = clump.longitude_idx + ew_data.size
    wav_scale = clump.scale_idx

    # The range should include the full clumps, and half of the scale size to either
    # side of the clump
...
#        print clump_params
        ...
        (left_deg, left_idx, right_deg, right_idx,
         clump_int_height, clump_fit_height,
         gauss_center, gauss_base,
         gauss_height, gauss_sigma) = fit_gaussian(tri_ew, left_idx, right_idx,
                                                   wav_center_idx)
...
#    print left_deg, right_deg, fit_width_idx, fit_width_deg, fit_height, clump_int_height, gauss_center, gauss_sigma, gauss_base, gauss_height
    return (left_deg, right_deg, fit_width_idx, fit_width_deg,
            clump_fit_height, clump_int_height, gauss_center, gauss_sigma,
            gauss_base, gauss_height )
