...
if arguments.historical_csv_filename:
    ...
    for et in np.arange(et1, et2, arguments.historical_step*86400):
        if arguments.pandora:
            min_dist, min_long = moons.pandora_close_approach(et, 0)
        else:
            min_dist, min_long = moons.prometheus_close_approach(et, 0)
...
for obs_id in f_ring.enumerate_obsids(arguments):
    ...
    if arguments.pandora:
        min_dist, min_long = moons.pandora_close_approach(mean_et, 0)
    else:
        min_dist, min_long = moons.prometheus_close_approach(mean_et, 0)
