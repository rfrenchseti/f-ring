...
#####################
    ...
    for zoom_level, hist_step, plot_step, plot_xmin, plot_xmax, plot_ymax in [('dim',0.2,1.,0,3,0.3001),('bright',0.5,1.,3,9,0.016),('full',0.5,2.,0,10,None)]:
        ...
        height_counts, bins, patches = plt.hist([v_clump_heights, c_clump_heights], bins,
                 weights = [v_height_weights, c_height_weights], label = ['Voyager', 'Cassini'],
                 color = [color_voyager, color_cassini], lw = 0.0)
    ...
    for zoom_level, hist_step, plot_step, plot_xmin, plot_xmax, plot_ymax in [('dim',0.025,0.1,0,0.301,0.25),('bright',0.05,0.2,0.3,1.1,0.04),('full',0.1,0.25,0,1.0,0.7)]:
        fig2 = plt.figure(figsize = (9,2))
        ax = fig2.add_subplot(111)
        plt.subplots_adjust(top = .95, bottom = 0.125, left = 0.08, right = 0.98)
...
#     save_fig(fig, ax, 'num_clumps_vs_time_'+fake_name+'.png')
    ...
    for spacecraft in ['Cassini', 'Voyager']:
        ...
        ax = fig.add_subplot(111)
        ax.set_xlabel('Weighted # of Clumps per Observation')
        ax.set_ylabel('Fractional # of Observations')
        ax.xaxis.tick_bottom()
        ax.yaxis.tick_left()
...
#         ax.get_xaxis().set_ticks(np.arange(bin_min, bin_max+step, step*2))
        ...
        ax.set_ylim(0, 0.5)#np.max(counts))
        ax.set_yticks([])            
