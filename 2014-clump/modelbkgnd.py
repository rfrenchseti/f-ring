...
def MaskImage(image, cutoff_sigmas=7., ignore_fraction=0.01, row_blur=5,
              debug=False):
    """Returns a copy of the mosaic, with a mask obscuring the pixels believed
    to be invalid.

    Inputs:
        mosaic      the original mosaic as an un-masked array.
    ...
    Return:         a new version of the array, with invalid pixels masked.
    """
    ...
    # Mask at exact values of zero - these are (almost certainly) pixels that were never initialized from
    # a reprojected image and thus are invalid by definition
    view = ma.masked_equal(view, 0.) 
    view.mask = ma.getmaskarray(view) # Necessary because the mask starts as 'nomask' first, not an array of bools
    if debug:
        initial_num_zeros = np.sum(view.mask) # How many invalid pixels we started with
        print 'Initial num zeros', initial_num_zeros
        print 'Rows', view.shape[0], 'Columns', view.shape[1], 'Total pixels', view.shape[0]*view.shape[1]
    ...
    while np.any(rows):
        ...
        # Locate excessive residuals in each active row (ignoring the really excessive pixels)
        mask_above1 = masked_mean_all + cutoff_sigmas * masked_sdev_all # 1-D array
        mask_below1 = masked_mean_all - cutoff_sigmas * masked_sdev_all # 1-D array

        # Smoothing: each row gets the maximum/minimum of its neighbors
        # Since we mask above the maximum (below the minimum) this will _reduce_ the amount of masking
        # we do here
        mask_above = filter.maximum_filter1d(mask_above1, row_blur, mode='nearest')
        mask_below = filter.minimum_filter1d(mask_below1, row_blur, mode='nearest')

        # Find the pixels in each row that are out of bounds
        deweight = (~save_mask & # Use the original mask 
                    ((view.data[rows,:] >= mask_above[rows][:,np.newaxis]) |  # np.newaxis just does a transpose
...
def ModelBackground(image, ring_rows=0.2, cutoff_sigmas=4, degree=2,
                    background_pixels=200, masked=False, debug=False, debug_col=None):
    """Derives a quadratic model for the background in each column of a mosaic.
    ...
    """
    if debug:
        print '*** In ModelBackground'
    
    def FitLinear(array, columns, x):
    ...
    def FitQuadratic(array, columns, x):
    ...
    ringless.mask = image.mask.copy() # But we are modifying the mask
#    print ring_rows
    ...
    if (type(background_pixels) == type(()) or
        type(background_pixels) == type([])):
    ...
    if type(below) == type(0.):
        below = int(below * rmin + 0.5)

    if type(above) == type(0.):
    ...
    reject = ((np.sum(~image.mask[0:rmin,:], axis=0) < below) |
              (np.sum(~image.mask[rmax+1:, :], axis=0) < above))
    
    if debug:
        print 'Reject col', np.sum(reject), 'Debug col', debug_col, ': #MaskBelow', np.sum(~image.mask[0:rmin, debug_col], axis=0),
        print '#MaskAbove', np.sum(~image.mask[rmax+1:, debug_col], axis=0),
    ...
    while np.any(column_active):
        ...
        # Fit the polynomial
        if degree == 1:
            model[:,column_active] = FitLinear(ringless, column_active, x)
        else:
            model[:,column_active] = FitQuadratic(ringless, column_active, x)
...
def IntegImage(masked, model, drad=5.):
    """Returns the mosaic converted to a longitudinal profile.

    Input:
        masked      a masked mosaic, as returned by MaskMosaic.
        model       a model mosaic, as returned by ModelMosiac.
        ring_row    a row number or fraction of the total rows that falls
                    within the radial range of the ring.
