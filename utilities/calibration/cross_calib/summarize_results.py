...
def print_mission_section(mission: str, by_moon: dict) -> dict[str, list[float]]:
    ...
    set1_cameras = sorted({r["camera"] for rows in by_moon.values()
                           for r in rows if r["voyager"] == mission})
    if mission.startswith("COISS") and set1_cameras:
        set1_str = ", ".join(f"COISS {c}" for c in set1_cameras)
        title = f"  SET1 = {set1_str.upper()}   SET2 = {mission.upper()}"
    else:
        title = f"  SET2 = {mission.upper()}"
    print(f"\n{'=' * total_width}")
    print(title)
    print(f"{'=' * total_width}")
