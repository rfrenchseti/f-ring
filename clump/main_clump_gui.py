...
# main_clump_gui.py
#
# Top-level entry point for the clump-finding GUI.
################################################################################
...
from imgdisp import ImageDisp, ScrolledList
...
################################################################################
#
# INITIALIZE MATPLOTLIB AESTHETICS
#
################################################################################
...
matplotlib.rc('axes',        facecolor=color_background,
                             edgecolor=color_foreground,
                             labelcolor=color_foreground)
matplotlib.rc('xtick',       color=color_foreground,
                             labelsize=20)
matplotlib.rc('xtick.major', size=15)
matplotlib.rc('xtick.minor', size=9)
matplotlib.rc('ytick',       color=color_foreground,
                             labelsize=20)
...
def command_load_clumps_from_file(clump_disp_data):
    clump_database_path, clump_chains_path = clump_util.clumpdb_paths(options)
    ...
    print('** Clump Database created with: Scale min', clump_find_arguments.scale_min,
...
def command_load_chains_from_file(clump_disp_data):
    clump_database_path, clump_chains_path = clump_util.clumpdb_paths(options)
    with open(clump_chains_path, 'rb') as clump_chains_fp:
        clump_find_options2 = pickle.load(clump_chains_fp)
        clump_chain_options = pickle.load(clump_chains_fp)
        (clump_disp_data.clump_db,
         clump_disp_data.clump_chain_list) = pickle.load(clump_chains_fp)
    print('** Clump Chain Database created with find options: Scale min',
                clump_find_options2.scale_min,
                'Scale max', clump_find_options2.scale_max,
                'Scale step', clump_find_options2.scale_step)
    print('*** Clumpsize min', clump_find_options2.clump_size_min,
                'Clumpsize max', clump_find_options2.clump_size_max,
                'Prefilter', clump_find_options2.prefilter)

    print('*** Clumps tracked with: Clumpsize min',
                clump_chain_arguments.clump_size_min,
                'Clumpsize max', clump_chain_arguments.clump_size_max,
                'Longitude tolerance', clump_chain_arguments.longitude_tolerance)
    print('*** Max movement', clump_chain_arguments.max_movement*86400,
                'Max time', clump_chain_arguments.max_time)
    print()
    clump_disp_data.cmd.update_clump_db(clump_disp_data.clump_db, redraw=False)
...
def command_load_approved_chain_list(clump_disp_data):
    ...
    elif not clump_disp_data.downsample_option  or not clump_disp_data.voyager_option:
        filename = 'matrix_clumps_list.pickle'

    approved_file = os.path.join(clump_util.ROOT, 'clump-data', filename)

    if os.path.exists(approved_file):
        with open(approved_file, 'rb') as approved_file_fp:
    ...
    print('LOADED APPROVED CHAIN LIST', filename, len(approved_list))
...
def command_save_approved_chain_list(clump_disp_data):
    ...
    approved_file = os.path.join(clump_util.ROOT, 'clump-data', filename)
    with open(approved_file, 'wb') as approved_file_fp:
        pickle.dump((clump_disp_data.clump_db, clump_disp_data.approved_chain_list),
                    approved_file_fp)
    clump_disp_data.approved_chain_list = []
    print('SAVED APPROVED CLUMP CHAIN LIST')
...
def command_create_chain_from_selected(clump_disp_data):
    ...
    (new_chain.rate, new_chain.base_long,
     res_fit) = clumputil.fit_rate(new_chain.clump_list)
    new_chain.long_err_list = res_fit
    clump_disp_data.clump_chain_list.append(new_chain)
    clump_disp_data.cmd.update_chain_list(clump_disp_data.clump_chain_list)

#
# Walk the clumps in the displayed region to build new chains based on the current
# parameters
#
def command_track_displayed_clumps(clump_disp_data):
    # First mark everything as ignored
    for obsid, clump_db_entry in list(clump_disp_data.clump_db.items()):
    ...
    for clump in clump_disp_data.cmd.displayed_clumps():
        if (clump_disp_data.var_chain_clumpsize_min.get() <=
                clump.scale <=
                    clump_disp_data.var_chain_clumpsize_max.get()):
    ...
    new_clump_chain_list = clumputil.track_clumps(
                clump_disp_data.clump_db,
                clump_disp_data.var_chain_max_rate.get()/86400.,
                clump_disp_data.var_chain_longitude_tolerance.get(),
                clump_disp_data.var_chain_max_time.get()*86400.,
                clump_disp_data.var_chain_scale_tolerance.get())
...
def clump_chain_info(chain):
    ...
    retstr += 'Straight-line rate: %e deg/sec = %.8f deg/day ' % (chain.rate,
                                                                  chain.rate*86400.)
    retstr += '(a=%.4f km)' % clump_util.RelativeRateToSemimajorAxis(chain.rate) + '\n'
...
#
# Display clumps in a selected chain
#
...
#        sub_size = len(chain.clump_list)
        ...
        for i, clump in enumerate(chain.clump_list):
            ...
            clumputil.plot_single_ew_profile(axes[i], clump.clump_db_entry,
                                             long_min, long_max,
                                             label=True, color=color_foreground)
            clumputil.plot_single_clump(axes[i], ew_data, clump,
                                        long_min, long_max, color= 'red')
...
#    clump_disp_data.sel_chains_copy = sel_chains
    if sel_chains is None:
        return
    for i, chain in enumerate(sel_chains):
...
def command_compare_clump_inert_peri(clump_disp_data):
    ...
    for chain in sel_chains:
        ...
        while i <= (len(chain.clump_list)-2):
            ...
            (clump1_inert, clump1_per, clump1_true) = clumputil.compare_inert_to_peri(
                            options, clump1.clump_db_entry.obsid,
                            clump1.g_center/longres1, clump1.g_center)
            (clump2_inert, clump2_per, clump2_true) = clumputil.compare_inert_to_peri(
                            options, clump2.clump_db_entry.obsid,
                            clump2.g_center/longres2, clump2.g_center)
            print('CLUMP 1 PERICENTRE:', clump1_per,
                  'ClUMP 1 INERTIAL LONG:', clump1_inert,
                  'CLUMP 1 TRUE:', clump1_true)
            print('CLUMP 2 PERICENTRE:', clump2_per,
                  'ClUMP 2 INERTIAL LONG:', clump2_inert,
                  'CLUMP 2 TRUE:', clump2_true)
            i += 1
...
def offrep_obsid_list_buttonrelease_handler(event, clump_disp_data):
    ...
    clump_disp_data.sel_chain = get_selected_chain_from_list(chain_selection,
                                                             clump_disp_data)
...
button_update = Button(left_control_frame, text='Create Chain From Selected',
                       command=lambda: command_create_chain_from_selected(
                                                                clump_disp_data))
button_update.grid(row=gridrow, column=0)
gridrow += 1
...
button_update = Button(left_control_frame,
                       text='Compare Inertial and Pericentre Longitudes',
...
clump_disp_data.cmd = ClumpMapDisp(clump_disp_data.clump_db,
                                   chain_list=clump_disp_data.clump_chain_list,
                                   parent=right_disp_frame)

#command_load_chains_from_file(clump_disp_data)

clump_disp_data.voyager_option = arguments.voyager
clump_disp_data.downsample_option = arguments.downsample
