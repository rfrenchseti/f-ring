...
class SDG(MotherWavelet):
    ...
    def get_coefs(self):
        ...
        xsd2 = -xi * xi / (self.scales[:,np.newaxis] * self.scales[:,np.newaxis])
        ...
        mw = c * (1. + xsd2) * np.exp(xsd2 / 2.)       #Second Derivative Gaussian
...
class Wavelet(object):
    ...
    def scalogram(self, show_coi=False, show_wps=False, ts=None, time=None,
        ...
        if use_period:
            ...
            y = self.motherwavelet.scales * .04 * 2 #XXX
        ...
        contf=ax1.contourf(x,y,np.abs(self.coefs)**2,100)
...
def cwt_nonfft(x, wavelet, weighting_function=lambda x: x**(-0.5), deep_copy=True,
    ...
    #
    # mother_wavelet = SDG(len_signal = len(data), scales = np.arange(10), normalize = True, fc = 'center')
    # wavelet = cwt_nonfft(data, mother_wavelet)
    # wave_coefs.scalogram()
    ...
    wt = np.zeros((len(wavelet.scales), len(x)))
    ...
    for pos in range(startx, endx):
        if (pos % 100) == 0:
            print(pos, '/', len(x))
        x_idx_min = max(pos-wavelet.coefs.shape[1]/2, 0)
        scale_idx_min = max(wavelet.coefs.shape[1]/2-pos, 0)
        scale_idx_max = min(x_idx_min+wavelet.coefs.shape[1], len(x))-x_idx_min
        scale_idx_len = scale_idx_max - scale_idx_min
        wt[:, pos] = np.sum(wavelet.coefs[:, scale_idx_min:scale_idx_max] *
                            x[x_idx_min:x_idx_min+scale_idx_max-scale_idx_min], axis=1)
        wt[:, pos] *= weighting_function(wavelet.scales)
